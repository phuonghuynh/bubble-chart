(function(root, factory) {
  ...
  $.extend(d3.svg.BubbleChart.prototype, {
    ...
    randomCirclesPositions: function(delta) {
      ...
      while (
        circles.length < self.items.length &&
        ++interval < self.intervalMax
      ) {
        var val = self.values[circles.length];
        var rad = Math.max(
          val * options.radiusMax / self.valueMax,
          options.radiusMin
        );
        var dist =
          self.innerRadius +
          rad +
          Math.random() * (self.outerRadius - self.innerRadius - rad * 2);
        ...
        if (!hit) {
          circles.push({
            ...
            cy: cy,
            r: rad,
            item: self.items[circles.length]
          });
        }
      }
      if (circles.length < self.items.length) {
        if (delta === options.radiusMin) {
          throw {
            message:
              "Not enough space for all bubble. Please change the options.",
            options: options
          };
        }
    ...
    setup: function() {
      ...
      self.svg = d3
        ...
        .attr("viewBox", function(d) {
          return ["0 0", options.viewBoxSize, options.viewBoxSize].join(" ");
        });
      self.circlePositions = self.randomCirclesPositions(
        options.intersectDelta
      );
      ...
      node
        ...
        .style("fill", function(d) {
          return options.data.color !== undefined
            ? options.data.color(d.item)
            : fnColor(d.item.text);
        })
        .attr("opacity", "0.8");
      node.sort(function(a, b) {
        return options.data.eval(b.item) - options.data.eval(a.item);
      });
    ...
    moveToCentral: function(node) {
      var self = this;
      var toCentralPoint = d3.svg.transform().translate(function(d) {
      ...
      self.transition.centralNode = node
        .classed({ active: true })
        .transition()
        .duration(self.options.transitDuration);
      self.transition.centralNode
    ...
    moveToReflection: function(node, swapped) {
      var self = this;
      var toReflectionPoint = d3.svg.transform().translate(function(d) {
    ...
    registerClickEvent: function(node) {
      ...
      node.style("cursor", "pointer").on("click", function(d) {
        ...
        self.moveToReflection(
          self.svg.selectAll(".node:not(.active)"),
          swapped
        );
        swapped = !swapped;
      });
